...
export default function Registration() {
  ...
  const [estudianteCodigoTelefono, setEstudianteCodigoTelefono] = useState<
    string | null
  >(null)
  const [estudianteTelefono, setEstudianteTelefono] = useState<string | null>(
    null
  )
  const [estudianteInstitucion, setEstudianteInstitucion] = useState<
    string | null
  >(null)
  const [estudianteOcupacion, setEstudianteOcupacion] = useState<string | null>(
    null
  )
  const [estudianteProfesion, setEstudianteProfesion] = useState<string | null>(
    null
  )
  const [estudianteLugarTrabajo, setEstudianteLugarTrabajo] = useState<
    string | null
  >(null)
  const [estudianteDireccion, setEstudianteDireccion] = useState<string | null>(
    null
  )
  const [estudianteEmail, setEstudianteEmail] = useState<string | null>(null)
  const [estudianteAlergias, setEstudianteAlergias] = useState<string | null>(
    null
  )
  const [estudianteAlergiasEspecificadas, setEstudianteAlergiasEspecificadas] =
    useState<string | null>(null)
  const [estudianteAntecedentes, setEstudianteAntecedentes] = useState<
    string | null
  >(null)
  const [estudianteContactoEmergencia, setEstudianteContactoEmergencia] =
    useState<string | null>(null)
  const [
    estudianteCodigoTelefonoEmergencia,
    setEstudianteCodigoTelefonoEmergencia,
  ] = useState<string | null>(null)
  const [estudianteTelefonoEmergencia, setEstudianteTelefonoEmergencia] =
    useState<string | null>(null)

  // State variables for legal representative's form fields
  const [representanteNombre, setRepresentanteNombre] = useState<string | null>(
    null
  )
  ...
  const [representanteParentesco, setRepresentanteParentesco] = useState<
    string | null
  >(null)
  const [representanteCodigoTelefono, setRepresentanteCodigoTelefono] =
    useState<string | null>(null)
  const [representanteTelefono, setRepresentanteTelefono] = useState<
    string | null
  >(null)
  const [representanteOcupacion, setRepresentanteOcupacion] = useState<
    string | null
  >(null)
  const [representanteProfesion, setRepresentanteProfesion] = useState<
    string | null
  >(null)
  const [representanteLugarTrabajo, setRepresentanteLugarTrabajo] = useState<
    string | null
  >(null)
  const [representanteDireccion, setRepresentanteDireccion] = useState<
    string | null
  >(null)
  const [representanteEmail, setRepresentanteEmail] = useState<string | null>(
    null
  )

  // State variables for additional form fields
  const [autorizacion, setAutorizacion] = useState<string | null>(null)
  const [instrumentos, setInstrumentos] = useState<Campo[]>([
    { id: uuid(), valor: "" },
  ])
  ...
  const handleGeneratePDF = async (): Promise<void> => {
    try {
      ...
      if (representanteCodigoTelefono && representanteTelefono) {
        var telefonoRepresentante =
          representanteCodigoTelefono + representanteTelefono
      }

      if (estudianteCodigoTelefonoEmergencia && estudianteTelefonoEmergencia) {
        var telefonoEmergencia =
          estudianteCodigoTelefonoEmergencia + estudianteTelefonoEmergencia
      }
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      form.checkValidity() &&
      studentRIFValidation &&
      representativeRIFValidation
    ) {
      setShowModal(true)
      handleGeneratePDF()
    }
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center w-[40%] gap-4 mb-4 imagen">
            ...
            <p className="text-[0.7rem] font-montserrat font-medium text-center text-gray-600 leading-snug">
              La foto debe ser nítida, tipo carnet, fondo blanco, tamaño 3x4 cm.
              Formato JPG o PNG.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              onChange={(e) =>
                setEstudianteAlergiasEspecificadas(e.target.value)
              }
            />
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex gap-2">
              <select
                ...
                onChange={(e) =>
                  setEstudianteCodigoTelefonoEmergencia(e.target.value)
                }
              >
                <option value="" disabled>
                  Seleccione una opción
              ...
              <input
                ...
                onChange={(e) =>
                  setEstudianteTelefonoEmergencia(e.target.value)
                }
              />
            </div>
          </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <div className="flex gap-2">
                <select
                  ...
                  onChange={(e) =>
                    setRepresentanteCodigoTelefono(e.target.value)
                  }
                  required={esMenor}
                >
                  <option value="" disabled>
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex flex-col gap-1">
              <label>
                Registro de Información Fiscal (RIF) {esMenor ? "*" : ""}
              </label>
              <input
                type="text"
                placeholder="eg. V123456789"
                ...
                {instrumentos.map((item, idx) => (
                  <div key={item.id} className="flex flex-col gap-1">
                    ...
                    <div className="flex gap-2">
                      <select
                        value={
                          instrumentos.filter((i) => i.id === item.id)[0]
                            ?.valor || ""
                        }
                        onChange={(e) =>
                          handleChangeInstrumento(item.id, e.target.value)
                        }
                        className="w-full"
                      >
                        <option value="" disabled>
                        ...
                        {(() => {
                          const selected = instrumentos
                            .filter((i) => i.id !== item.id)
                            .map((i) => i.valor)
                            .filter(Boolean)

                          const filtered = listadoInstrumentos.filter(
                            (catedra) => !selected.includes(catedra)
                          )

                          const includeCurrent =
                            item.valor && !filtered.includes(item.valor)
                              ? [item.valor]
                              : []
                ...
                {teoricas.map((item, idx) => (
                  <div key={item.id} className="flex flex-col gap-1">
                    ...
                    <div className="flex gap-2">
                      <select
                        value={
                          teoricas.filter((i) => i.id === item.id)[0]?.valor ||
                          ""
                        }
                        onChange={(e) =>
                          handleChangeTeoricas(item.id, e.target.value)
                        }
                        className="w-full"
                      >
                        <option value="" disabled>
                        ...
                        {(() => {
                          const selected = teoricas
                            .filter((i) => i.id !== item.id)
                            .map((i) => i.valor)
                            .filter(Boolean)

                          const filtered = listadoTeoricas.filter(
                            (catedra) => !selected.includes(catedra)
                          )
                ...
                {otros.map((item, idx) => (
                  <div key={item.id} className="flex flex-col gap-1">
                    ...
                    <div className="flex gap-2">
                      <select
                        value={
                          otros.filter((i) => i.id === item.id)[0]?.valor || ""
                        }
                        onChange={(e) =>
                          handleChangeOtros(item.id, e.target.value)
                        }
                        className="w-full"
                      >
                        <option value="" disabled>
                        ...
                        {(() => {
                          const selected = otros
                            .filter((i) => i.id !== item.id)
                            .map((i) => i.valor)
                            .filter(Boolean)

                          const filtered = listadoOtros.filter(
                            (catedra) => !selected.includes(catedra)
                          )
          ...
          <p className="text-xs md:text-sm font-montserrat font-medium text-justify">
            ...
            del material fotográfico y audiovisual de las actividades académicas
            y artísticas que se lleven a cabo durante el desarrollo del Academia
            Internacional de Música. Las imágenes podrán ser usadas para la
